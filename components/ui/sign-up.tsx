...
export const AuthComponent = ({ logo = <DefaultLogo />, brandName = "EaseMize" }: AuthComponentProps) => {
                    ...
                    <motion.div initial={{ scale: 0.9, opacity: 0 }} animate={{ scale: 1, opacity: 1 }} exit={{ scale: 0.9, opacity: 0 }} className="relative bg-card border-4 border-border rounded-2xl p-8 w-full max-w-sm flex flex-col items-center gap-4 mx-2 overflow-hidden shadow-2xl">
                        ...
                        {modalStatus === 'success' &&
                            <div className="flex flex-col items-center gap-4">
                                ...
                                <p className="text-lg font-bold text-foreground">{modalSteps[modalSteps.length - 1].message}</p>
                            </div>
                        }
                    </motion.div>
                </motion.div>
    ...
    return (
        <div className="bg-background min-h-screen w-full flex flex-col relative overflow-hidden">
            <style>{`
            ...
            input:-webkit-autofill, input:-webkit-autofill:hover, input:-webkit-autofill:focus, input:-webkit-autofill:active { -webkit-box-shadow: 0 0 0 30px white inset !important; -webkit-text-fill-color: black !important; transition: background-color 5000s ease-in-out 0s !important; }
            ...
            .glass-button-shadow { --shadow-cutoff-fix: 2em; position: absolute; width: calc(100% + var(--shadow-cutoff-fix)); height: calc(100% + var(--shadow-cutoff-fix)); top: calc(0% - var(--shadow-cutoff-fix) / 2); left: calc(0% - var(--shadow-cutoff-fix) / 2); filter: blur(clamp(2px, 0.125em, 12px)); pointer-events: none; z-index: 0; }
            .glass-button { backdrop-filter: blur(4px); transition: all var(--anim-time) var(--anim-ease); background: rgba(255,255,255,0.1); border: 1px solid rgba(255,255,255,0.2); }
            .glass-input-wrap { position: relative; z-index: 2; border-radius: 9999px; }
            .glass-input { display: flex; width: 100%; align-items: center; gap: 0.5rem; border-radius: 9999px; padding: 0.5rem; background: rgba(255,255,255,0.8); border: 1px solid #e2e8f0; transition: all 0.3s ease; }
            .glass-input:focus-within { border-color: #5B5CF6; box-shadow: 0 0 0 2px rgba(91, 92, 246, 0.1); }
        `}</style>
            ...
            <div className="fixed top-6 left-6 z-20 flex items-center gap-3">
                <div className="bg-primary text-primary-foreground p-2 rounded-xl shadow-lg shadow-primary/20">{logo}</div>
                <h1 className="text-xl font-black tracking-tight text-gray-900">{brandName}</h1>
            </div>

            <div className="flex-1 flex items-center justify-center p-6 relative">
                <div className="absolute inset-0 z-0 opacity-40"><GradientBackground /></div>

                <fieldset disabled={modalStatus !== 'closed'} className="relative z-10 w-full max-w-md mx-auto">
                    <AnimatePresence mode="wait">
                        {authStep === "email" && <motion.div key="email-content" initial={{ y: 20, opacity: 0 }} animate={{ y: 0, opacity: 1 }} exit={{ opacity: 0, y: -20 }} transition={{ duration: 0.4 }} className="space-y-8">
                            <div className="text-center space-y-2">
                                <h2 className="text-4xl md:text-5xl font-black tracking-tight text-gray-900 leading-tight">Join the future <br />of insights.</h2>
                                <p className="text-gray-500 font-semibold tracking-wide uppercase text-xs">Create your account to get started</p>
                            </div>

                            <div className="grid grid-cols-2 gap-4">
                                <button className="flex items-center justify-center gap-3 p-3 bg-white border border-gray-100 rounded-2xl font-bold text-sm hover:bg-gray-50 transition-all active:scale-95 shadow-sm"><GoogleIcon /> <span>Google</span></button>
                                <button className="flex items-center justify-center gap-3 p-3 bg-gray-900 border border-gray-800 rounded-2xl font-bold text-sm text-white hover:bg-black transition-all active:scale-95 shadow-lg shadow-black/10"><GitHubIcon /> <span>GitHub</span></button>
                            </div>

                            <div className="flex items-center gap-4"><hr className="flex-1 border-gray-100" /><span className="text-[10px] font-black text-gray-300 tracking-widest uppercase">OR CONTINUE WITH</span><hr className="flex-1 border-gray-100" /></div>
                        </motion.div>}

                        {authStep === "password" && <motion.div key="password-title" initial={{ y: 20, opacity: 0 }} animate={{ y: 0, opacity: 1 }} exit={{ opacity: 0, y: -20 }} className="text-center space-y-2">
                            <h2 className="text-4xl font-black tracking-tight text-gray-900 leading-tight">Secure your <br />identity.</h2>
                            <p className="text-gray-500 font-semibold text-sm">Choose a strong password (6+ chars)</p>
                        </motion.div>}

                        {authStep === "confirmPassword" && <motion.div key="confirm-title" initial={{ y: 20, opacity: 0 }} animate={{ y: 0, opacity: 1 }} exit={{ opacity: 0, y: -20 }} className="text-center space-y-2">
                            <h2 className="text-4xl font-black tracking-tight text-gray-900 leading-tight">One last <br />check.</h2>
                            <p className="text-gray-500 font-semibold text-sm">Verify your password to complete setup</p>
                        </motion.div>}
                    </AnimatePresence>
                    ...
                    <form onSubmit={handleFinalSubmit} className="mt-8 space-y-4">
                        <AnimatePresence mode="wait">
                            {authStep !== 'confirmPassword' ? (
                                <motion.div key="stage-1" exit={{ opacity: 0 }} className="space-y-4">
                                    <div className="glass-input-wrap">
                                        <div className="glass-input">
                                            <div className="w-10 flex justify-center"><Mail className="w-5 h-5 text-gray-400" /></div>
                                            <input type="email" placeholder="Email address" value={email} onChange={(e) => setEmail(e.target.value)} onKeyDown={handleKeyDown} className="flex-1 bg-transparent font-semibold text-gray-900 border-none focus:ring-0 placeholder:text-gray-400" />
                                            {isEmailValid && authStep === 'email' && <motion.button initial={{ scale: 0 }} animate={{ scale: 1 }} type="button" onClick={handleProgressStep} className="bg-primary text-white p-2 rounded-full shadow-lg shadow-primary/20"><ArrowRight className="w-4 h-4" /></motion.button>}
                                        </div>
                                    </div>

                                    {authStep === "password" && <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} className="space-y-4">
                                        <div className="glass-input">
                                            <div className="w-10 flex justify-center"><Lock className="w-5 h-5 text-gray-400" /></div>
                                            <input ref={passwordInputRef} type={showPassword ? "text" : "password"} placeholder="Password" value={password} onChange={(e) => setPassword(e.target.value)} onKeyDown={handleKeyDown} className="flex-1 bg-transparent font-semibold text-gray-900 border-none focus:ring-0 placeholder:text-gray-400" />
                                            <button type="button" onClick={() => setShowPassword(!showPassword)} className="p-2 text-gray-400 hover:text-gray-600 transition-colors">{showPassword ? <EyeOff className="w-5 h-5" /> : <Eye className="w-5 h-5" />}</button>
                                            {isPasswordValid && <motion.button initial={{ scale: 0 }} animate={{ scale: 1 }} type="button" onClick={handleProgressStep} className="bg-primary text-white p-2 rounded-full shadow-lg shadow-primary/20 ml-2"><ArrowRight className="w-4 h-4" /></motion.button>}
                                        </div>
                                        <button type="button" onClick={handleGoBack} className="flex items-center gap-2 text-xs font-bold text-gray-400 hover:text-gray-900 transition-colors uppercase tracking-widest"><ArrowLeft className="w-3 h-3" /> Step back</button>
                                    </motion.div>}
                                </motion.div>
                            ) : (
                                <motion.div key="stage-2" initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} className="space-y-4">
                                    <div className="glass-input">
                                        <div className="w-10 flex justify-center"><Lock className="w-5 h-5 text-gray-400" /></div>
                                        <input ref={confirmPasswordInputRef} type={showConfirmPassword ? "text" : "password"} placeholder="Verify Password" value={confirmPassword} onChange={(e) => setConfirmPassword(e.target.value)} className="flex-1 bg-transparent font-semibold text-gray-900 border-none focus:ring-0 placeholder:text-gray-400" />
                                        <button type="button" onClick={() => setShowConfirmPassword(!showConfirmPassword)} className="p-2 text-gray-400 hover:text-gray-600 transition-colors">{showConfirmPassword ? <EyeOff className="w-5 h-5" /> : <Eye className="w-5 h-5" />}</button>
                                        {isConfirmPasswordValid && <motion.button initial={{ scale: 0 }} animate={{ scale: 1 }} type="submit" className="bg-primary text-white p-2 rounded-full shadow-lg shadow-primary/20 ml-2"><ArrowRight className="w-4 h-4" /></motion.button>}
                                    </div>
                                    <button type="button" onClick={handleGoBack} className="flex items-center gap-2 text-xs font-bold text-gray-400 hover:text-gray-900 transition-colors uppercase tracking-widest"><ArrowLeft className="w-3 h-3" /> Step back</button>
                                </motion.div>
                            )}
