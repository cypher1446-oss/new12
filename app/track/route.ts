...
export async function GET(request: NextRequest) {
    ...
    try {
        ...
        if (project.pid_prefix) {
            const { data: generatedPid, error: pidError } = await supabase
                .rpc('fn_generate_next_client_pid', { target_project_id: project.id })

            if (pidError) {
                console.error('PID Generation failed:', pidError)
                // Fallback to project_code or abort? 
                // Given the strict requirement for no duplicates/exposure, we should probably abort or log heavily.
                // For now, we'll try to continue with project_code if generatedPid is null but log the error.
            }
            clientPid = generatedPid
        }
